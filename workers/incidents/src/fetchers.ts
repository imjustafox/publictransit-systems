// Incident feed fetchers, ported verbatim from the app's src/lib/data.ts so the
// worker polls the agencies and the app only ever reads our KV. Environment
// neutral: plain fetch, no Next.js cache options, no filesystem.

import GtfsRealtimeBindings from "gtfs-realtime-bindings";
import type { IncidentData, ServiceAlert, UnitOutage } from "./types";
...
// Station name index for NYC Subway, built from stations.json bundled into
// the worker at deploy time.
import nycStationsJson from "../../../data/systems/nyc-subway/stations.json";
...
async function getNycStationIndex(): Promise<Map<string, Array<{ id: string; lines: string[] }>>> {
  ...
  const stations = (
    nycStationsJson as {
      stations: Array<{ id: string; name: string; lines: string[]; status: string }>;
    }
  ).stations;
...
export async function fetchBartIncidents(): Promise<IncidentData | null> {
  try {
...
export async function fetchSoundTransitIncidents(): Promise<IncidentData | null> {
...
export async function fetchNycSubwayIncidents(): Promise<IncidentData | null> {
  try {
...
// Plain http on purpose: the server's TLS config is incompatible with the
// workers runtime (fetch dies with "internal error"), and the feed is public
// data served identically over http.
const MTA_MARYLAND_ALERTS_URL = "http://feeds.mta.maryland.gov/alerts.pb";
...
async function fetchMtaMarylandFeed(): Promise<GtfsRealtimeBindings.transit_realtime.FeedMessage | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`MTA Maryland feed returned ${response.status}`);
      return null;
    }
    ...
    console.error("MTA Maryland feed fetch failed:", error);
    return null;
  }
}
...
export async function fetchBaltimoreMetroIncidents(): Promise<IncidentData | null> {
...
export async function fetchBaltimoreLightRailIncidents(): Promise<IncidentData | null> {
  return fetchBaltimoreIncidents(
...
export async function fetchCtaIncidents(): Promise<IncidentData | null> {
  try {
...
// The id map and station names are bundled into the worker at deploy time.
import rtdIdMap from "../../../data/systems/rtd-denver/id_map.json";
import rtdStations from "../../../data/systems/rtd-denver/stations.json";
...
const rtdStopMap: Record<string, string> = (rtdIdMap as { stations: Record<string, string> })
  .stations;

// Longest first so "Littleton-Downtown" wins over any shorter overlap.
const rtdStationNames: Array<{ name: string; id: string }> = (
  rtdStations as { stations: Array<{ id: string; name: string }> }
).stations
  .map((st) => ({ name: st.name, id: st.id }))
  .sort((a, b) => b.name.length - a.name.length);

export async function fetchRtdIncidents(): Promise<IncidentData | null> {
  ...
  for (const entity of feed.entity || []) {
    ...
    for (const informed of alert.informedEntity || []) {
      ...
      const stationId = informed.stopId ? rtdStopMap[informed.stopId] : undefined;
    ...
    if (affectedStations.length === 0) {
      ...
      for (const st of rtdStationNames) {
