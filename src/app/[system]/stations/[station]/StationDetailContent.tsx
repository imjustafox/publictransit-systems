...
// Shared body of the flat and network-scoped station detail pages.
export async function StationDetailContent({
  systemId,
  stationId,
  network,
  ...
  const stationsHref = network ? `/${systemId}/${network.id}/stations` : `/${systemId}/stations`;
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm font-mono">
        ...
        <Link href={stationsHref} className="text-text-muted hover:text-accent-secondary">
          Stations
        </Link>
        <span className="text-text-muted">/</span>
